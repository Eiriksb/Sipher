...
- 🎙️ **Live captions** for [Simple Voice Chat](https://modrinth.com/plugin/simple-voice-chat). Captions appear while you are still talking and settle into a final sentence when you pause or release push-to-talk.
...
- 🗺️ **Menus in 16 languages**, and a welcome screen on first start that sets up your language in one click.
...
2. On first start, pick your language on the welcome screen (and download its pack if it isn't English), and choose whether to share your captions.
3. Join a world and talk on voice chat. Captions appear above your head (in third person) and in the transcript box.
4. Press **O** (or *Mods → Sipher → Config*) for settings: turn captions on/off, choose whether to share them, bubble size and colours, transcript box, and *Copy diagnostics* for bug reports.
5. Want to speak one language and read another? Open **Languages…** in the settings.
...
- **Language packs contain data only** — machine-learning model weights, vocabularies and licence texts. They never contain code or programs: all code, including native libraries, is inside the mod file you install from Modrinth. The downloader refuses any other kind of file.
...
- sherpa-onnx (Apache-2.0) and ONNX Runtime (MIT)
- Moonshine English speech recognition (MIT) and Silero VAD (MIT)
- OPUS-MT translation models by the University of Helsinki (Apache-2.0 / CC-BY-4.0)
- NVIDIA NeMo speech models (CC-BY-4.0; NVIDIA Open Model License for Danish), SenseVoice (FunASR Model License), [NB-Whisper](https://huggingface.co/NbAiLab) by the National Library of Norway (Apache-2.0), [KB-Whisper](https://huggingface.co/KBLab) by the National Library of Sweden (Apache-2.0), GigaAM by SaluteDevices (MIT) and [ElanMT](https://huggingface.co/Mitsua) by Mitsua (CC-BY-SA-4.0)

Licences and links for everything built into the mod: [THIRD_PARTY_NOTICES.md](https://github.com/Eiriksb/Sipher/blob/main/THIRD_PARTY_NOTICES.md). Each language pack shows its models' licences before you download it and includes them afterwards. Source code and the issue tracker are linked on this page.
